...
import com.easyplay.easygame.activity.ImageBrowserActivity;
import com.easyplay.easygame.activity.LocationActivity;
import com.easyplay.easygame.tools.ImageLoadOptions;
import com.easyplay.easygame.util.FaceTextUtils;
import com.easyplay.easygame.util.TimeUtil;
import com.easyplay.easygame.view.ViewHolder;
...
public class MessageChatAdapter extends BaseListAdapter<BmobMsg> {
  ...
  public MessageChatAdapter(Context context, List<BmobMsg> msgList) {
    ...
    options = new DisplayImageOptions.Builder()
        ...
        .showImageOnFail(R.drawable.ic_launcher).resetViewBeforeLoading(true)
  ...
  private View createViewByType(BmobMsg message, int position) {
    ...
    if (type == BmobConfig.TYPE_IMAGE) {// 图片类型
      return getItemViewType(position) == TYPE_RECEIVER_IMAGE ? mInflater
          .inflate(R.layout.item_chat_received_image, null) : mInflater
          .inflate(R.layout.item_chat_sent_image, null);
    } else if (type == BmobConfig.TYPE_LOCATION) {// 位置类型
      return getItemViewType(position) == TYPE_RECEIVER_LOCATION ? mInflater
          .inflate(R.layout.item_chat_received_location, null) : mInflater
          .inflate(R.layout.item_chat_sent_location, null);
    } else if (type == BmobConfig.TYPE_VOICE) {// 语音类型
      return getItemViewType(position) == TYPE_RECEIVER_VOICE ? mInflater
          .inflate(R.layout.item_chat_received_voice, null) : mInflater
          .inflate(R.layout.item_chat_sent_voice, null);
    } else {// 剩下默认的都是文本
  ...
  public View bindView(final int position, View convertView, ViewGroup parent) {
    ...
    case BmobConfig.TYPE_VOICE:// 语音消息
      try {
        ...
        // 播放语音文件
        iv_voice.setOnClickListener(new NewRecordPlayClickListener(mContext,
            item, iv_voice));
      } catch (Exception e) {
  ...
  private void dealWithImage(int position, final ProgressBar progress_load,
      ImageView iv_fail_resend, TextView tv_send_status, ImageView iv_picture,
      BmobMsg item) {
    String text = item.getContent();
