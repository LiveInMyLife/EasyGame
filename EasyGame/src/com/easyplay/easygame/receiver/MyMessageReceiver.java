package com.easyplay.easygame.receiver;
...
import com.easyplay.easygame.activity.MainActivity;
...
public class MyMessageReceiver extends BroadcastReceiver {
    ...
    try {
      ...
      if (tag.equals(BmobConfig.TAG_OFFLINE)) {// 下线通知
        ...
        if (fromId != null && !BmobDB.create(context, toId).isBlackUser(fromId)) {// 该消息发送方不为黑名单用户
          if (TextUtils.isEmpty(tag)) {// 不携带tag标签--此可接收陌生人的消息
                ...
                new OnReceiveListener() {
                  ...
                  public void onSuccess(BmobMsg msg) {
                    ...
                    if (ehList.size() > 0) {// 有监听的时候，传递下去
                      ...
                      boolean isAllow = BaseApplication.getInstance()
                          .getSpUtil().isAllowPushNotify();
            ...
            if (tag.equals(BmobConfig.TAG_ADD_CONTACT)) {
              ...
              BmobUserManager.getInstance(context).addContactAfterAgree(
                  username, new FindListener<BmobChatUser>() {
          ...
          BmobChatManager.getInstance(context).updateMsgReaded(true, fromId,
              msgTime);
  ...
  public void showMsgNotify(Context context, BmobMsg msg) {
    ...
    boolean isAllowVoice = BaseApplication.getInstance().getSpUtil()
        .isAllowVoice();
    ...
    BmobNotifyManager.getInstance(context).showNotifyWithExtras(isAllowVoice,
        isAllowVibrate, icon, tickerText.toString(), contentTitle,
        tickerText.toString(), intent);
  }
  ...
  public void showOtherNotify(Context context, String username, String toId,
    ...
    boolean isAllow = BaseApplication.getInstance().getSpUtil()
        .isAllowPushNotify();
    boolean isAllowVoice = BaseApplication.getInstance().getSpUtil()
        .isAllowVoice();
    boolean isAllowVibrate = BaseApplication.getInstance().getSpUtil()
