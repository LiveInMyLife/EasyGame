...
public class LocationActivity extends BaseActivity implements
  ...
  private void initBaiduMap() {
    ...
    if (type.equals("select")) {// 选择发送位置
      ...
      // 显示当前位置图标
      OverlayOptions ooA = new MarkerOptions().position(latlng).icon(bdgeo)
          .zIndex(9);
  ...
  private void initLocClient() {
    ...
    mBaiduMap.setMyLocationConfigeration(new MyLocationConfigeration(
        com.baidu.mapapi.map.MyLocationConfigeration.LocationMode.NORMAL, true,
        null));
    // 定位初始化
  ...
  public class MyLocationListenner implements BDLocationListener {
    ...
    public void onReceiveLocation(BDLocation location) {
      ...
      BmobLog.i("lontitude = " + location.getLongitude() + ",latitude = "
          + location.getLatitude() + ",地址 = " + lastLocation.getAddrStr());
  ...
  public class BaiduReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      if (s
        ...
        showErrorToast("key 验证出错! 请在 AndroidManifest.xml 文件中检查 key 设置");
      } else if (s
